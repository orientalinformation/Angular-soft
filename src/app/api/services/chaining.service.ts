...
export class ChainingService extends BaseService {
   ...
   * @param id - id study
   */
  clearParentChainingResponse(id: number): Observable<HttpResponse<ModelChaining>> {
    let __params = this.newParams();
    let __headers = new HttpHeaders();
    let __body: any = null;
    ...
    let req = new HttpRequest<any>(
      "GET",
      this.rootUrl + `/chaining/clear/${id}`,
      __body,
      {
        headers: __headers,
        params: __params,
   ...
   * @param id - id study
   */
  clearParentChaining(id: number): Observable<ModelChaining> {
    return this.clearParentChainingResponse(id).pipe(
      map(_r => _r.body)
    );
  }}
