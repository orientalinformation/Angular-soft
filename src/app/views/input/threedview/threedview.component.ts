...
export class ThreedviewComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  drawStandOVal() {
    ...
    for (let index = 0; index < this.model.elements.length; index++) {
      ...
      const path = new THREE.Shape();
      path.ellipse(0, 0, element.SHAPE_PARAM1, element.SHAPE_PARAM3, 0, Math.PI * 2, false, 0);
      // geometry = new THREE.ShapeBufferGeometry( path );
  ...
  ___drawStandOVal() {
    ...
    let geometry = null;
    geometry = new THREE.ExtrudeBufferGeometry( shape1, extrudeSettings );
    const material = new THREE.MeshBasicMaterial( { color: 0x00ff00 } );
    const mesh = new THREE.Mesh(geometry, material);
    this.scene.add(mesh);
    this.boundingRadius = Math.max(width, length);
    // this.cameraTarget = new THREE.Vector3(0, 0, 0);
  }
