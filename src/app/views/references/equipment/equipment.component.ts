...
export class EquipmentComponent implements OnInit, AfterViewInit {
  ...
  public convectionSpeedSymbol = 'Hz';
  ...
  saveNewEquipment(typeCalculate = 0) {
    ...
    if (Number(this.statusEquip) === 0) {
      ...
      if (!this.selectEquipGener.checkFanFrequency) {
        this.newEquipment.fanFrequency = 0;
      }
  ...
  checkEquipment(equipment, check) {
    ...
    if (this.selectEquipGener.checkFanFrequency) {
      if (isNullOrUndefined(equipment.fanFrequency) || String(equipment.fanFrequency) === '' || isNaN(equipment.fanFrequency)) {
        this.toastr.error(this.translate.instant('Please specify fan frequency'), 'Error');
        return;
      }
    }
